...
class KDTree:
    ...
    # vcvicek Fixed radius search for a point, but only interested in existence

    def search_exists(self, center, radius):
        """Find out if there exists a point within radius of center.
        ...
        if center.shape!=(self.dim,):
                raise Exception("Expected a %i-dimensional NumPy array" \
                                % self.dim)
        self.kdt.search_exists_center_radius(center, radius)
...
#    print "Found %i point pairs within radius %f." % (len(indices), query_radius)
    ...
    for i in range(0, 100):
        ...
        center=200*random(dim)
        
        # search neighbors
#        kdtree.search(center, query_radius) # this was the old query for all the points in the searched radius
        kdtree.search_exists(center, query_radius) # this is my query which only returns one point in the searched radius
