...
class patch:
  ...
  def write3d(self,file):
    ...
    for molecule in self.molecules:
      ...
      # unpack tris in molecule before atoms so idatom = all previous atoms
      
      for tri in molecule[2]:
        idtri += 1
        tris.append([idtri,tri[0],tri[1]+idatom+1,tri[2]+idatom+1,
                    tri[3]+idatom+1])
    ...
    # kludge for tri add-on, data.py is not writing tri info
    #   need to write out new section of data file
    # how to allow for aotm line format for hybrid atom_style?

    if tris:
      lines = []
      for tri in tris:
        line = "%d %d %d %d %d\n" % (tri[0], tri[1], tri[2], tri[3], tri[4])
        lines.append(line)
        d.sections["Tris"] = lines
  ...
  def write2d(self,file):
    ...
    for molecule in self.molecules:
      ...
      # xnew,ynew,xnew = coeffs in new rotated basis vectors
      # x,y,z = coeffs in original xyz axes
      # kludge for switching between 2 atom styles: molecular,line
      #   based on presence of segments
      #   doesn't allow mixing of molecule types
      #   need to write out new section of data file
      #   how to allow format for hybrid atom style
      #   probably need some user-set info to control this
      ...
      if not molecule[3]:
        ...
        for i,atom in enumerate(molecule[0]):
          ...
          segment = molecule[3][i]
          dx = segment[2]-segment[0]
          dy = segment[3]-segment[1]
          length = sqrt(dx*dx + dy*dy)
          xctr = 0.5*(segment[0]+segment[2])
          yctr = 0.5*(segment[1]+segment[3])

          xrot = xorig + xctr*xp[0] + yctr*yp[0]
          yrot = yorig + xctr*xp[1] + yctr*yp[1]

          xnew = xctr + segment[0]
          ynew = yctr + segment[1]
          segx = xorig + xnew*xp[0] + ynew*yp[0]
          segy = yorig + xnew*xp[1] + ynew*yp[1]
          segment[0] = segx - xrot
          segment[1] = segy - yrot
          ...
          segment[2] = segx - xrot
          segment[3] = segy - yrot
          atoms.append([idatom,idmol,atom[0],length,1.0,x,y,z,
                        segment[0],segment[1],segment[2],segment[3],ix,iy,iz])
    ...
    # atoms section of data file
    # kludge based on nfields to format %d,%g for 2 atom styles: molecular,line
    ...
    for atom in atoms:
      if nfields == 9:
        ...
        line = "%d %d %d %g %g %g %g %g %g %g %g %g %d %d %d\n" % \
            (atom[0], atom[1], atom[2], atom[3], atom[4], atom[5],
             atom[6], atom[7], atom[8], atom[9], atom[10], atom[11],
             atom[12], atom[13], atom[14])
      lines.append(line)
    d.sections["Atoms"] = lines
  ...
  def tritet(self,*params):
    ...
    atoms.append([type,v/2,v/2,v/2])
    atoms.append([type,-v/2,-v/2,v/2])
    atoms.append([type,-v/2,v/2,-v/2])
    atoms.append([type,v/2,-v/2,-v/2])

    tris.append([type+1,0,1,2])
    tris.append([type,3,1,0])
    tris.append([type,0,2,3])
    tris.append([type,3,2,1])
  ...
  # type = type of each vertex in cube
  
  def tricube(self,*params):
    a = params[0]
    type = params[1]
    v = 0.5*a
    ...
    tris.append([type+1,0,4,6])
    tris.append([type,0,6,2])
    tris.append([type,1,3,7])
    tris.append([type,1,7,5])
    tris.append([type,0,1,5])
    tris.append([type,0,5,4])
    tris.append([type,2,6,7])
    tris.append([type,2,7,3])
    tris.append([type,0,2,3])
    tris.append([type,0,3,1])
    tris.append([type,4,5,7])
    tris.append([type,4,7,6])
  ...
  def linebox(self,*params):
    ...
    segments.append([-a/2,-b/2,a/2,-b/2])
    segments.append([a/2,-b/2,a/2,b/2])
    segments.append([a/2,b/2,-a/2,b/2])
    segments.append([-a/2,b/2,-a/2,-b/2])
