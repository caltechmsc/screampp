...
- **Multiple Interfaces**:
  ...
  - **Native Rust Crate**: The core library (`screampp`) is available as a Git dependency for direct use in other Rust-based scientific computing projects.
...
```toml
[dependencies]
screampp = { git = "https://github.com/caltechmsc/screampp" }
```
...
- **For Developers**:
  - **Rust Library API Docs**: API documentation for the `scream-core` (`screampp`) crate can be built locally with `cargo doc --open`.
